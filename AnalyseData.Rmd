...
## Explore the perceptual data
Do a jitter and scatter plot of the perceptual dimensions
1. Hc - Hue quadrature. Varies from 0 to 400: redness (0) - yellowness (100) - greenness (200) - blueness (300) - redness (400)
2. Jc - Lightness. A scale of 0–100 relative to the brightness of reference white
3. Mc - Colorfulness. An absolute scale of 0 to unlimited, but typically 0 to 100

```{r perceptions}
ggplot(data=all)+geom_jitter(aes(y=Jc,x="Jc"))+geom_jitter(aes(y=Mc,x="Mc"))+geom_jitter(aes(y=Hc,x="Hc"))+ theme(axis.title.x = element_blank())+ theme(axis.title.y = element_blank())

ggplot(data=all)+aes(x=Hc, y=Jc, color=Mc)+geom_point()
