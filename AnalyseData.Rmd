...
date: "07/08/2017"
---
# High Luminance Color Space Model 
...
It is based on the latest research into extended luminance color appearance models beyond CIECAM02.

## References
* Kim Min H, **Modeling Human Color Perception under Extended Luminance Levels**, http://reality.cs.ucl.ac.uk/projects/xlrcam/kim09xlrcam.pdf
...
all$xw = all$Xw / (all$Xw + all$Yw + all$Zw)
all$yw = all$Yw / (all$Xw + all$Yw + all$Zw)
all$n = (all$xw - 0.3320) / (all$yw - 0.1858)
all$CCT1 = -449*all$n^3 + 3525*all$n^2 - 6823.3*all$n + 5520.33
...
all <- subset(all, select = -c(xw, yw, n, CCT1, Xb1, Yb1, Zb1))
...
## convert LMSs (color sample) and LMSw (adopted white) to LMS space
## LMSs = XYZs converted to "spectrally sharpened" CAT02 LMS space to prepare for adaptation.
## LMSw = XYZw (adopted white) converted to "spectrally sharpened" CAT02 LMS space to prepare for adaptation.
XYZs <- matrix(c(all$Xs,all$Ys,all$Zs), ncol=3)
XYZw <- matrix(c(all$Xw,all$Yw,all$Zw), ncol=3)
LMSs <- t(MCAT02 %*% t(XYZs))
LMSw <- t(MCAT02 %*% t(XYZw))

## use CAT02 to adapt from the illumant adopted white to a reference white used in CIECAM02
## F = factor determining degree of adaptation based on surround condition (Average, Dim, Dark)
## D = degree of adaptation (0 for no adaptation, 1 for full adaption)
## LMSc = Chromatically adaptation XYZs sample using CAT02
all$F[all$Ambient=='dark'] <- 0.8
all$F[all$Ambient=='dim'] <- 0.9
all$F[all$Ambient=='average'] <- 1.0
all$D <- all$F*(1 - (1/3.6)*exp((-all$La-42)/92))
Lc <- LMSs[,1] * (all$D * all$Yw / LMSw[,1] + 1 - all$D)
Mc <- LMSs[,2] * (all$D * all$Yw / LMSw[,2] + 1 - all$D)
Sc <- LMSs[,3] * (all$D * all$Yw / LMSw[,3] + 1 - all$D)
